...
import com.epam.taxopark.data.ContentRetrieve;
import com.epam.taxopark.data.DBContentRetrieve;
import com.epam.taxopark.data.JsonContentRetrieve;
import com.epam.taxopark.vehicles.vehicletypes.taxoparkutils.TaxoparkUtils;
import com.epam.taxopark.data.XMLContentRetrieve;
...
//      Retrieve List records from Derby DB
        ContentRetrieve dbContentRetrieve = new DBContentRetrieve();
        try {
            //dbContentRetrieve.dbContent();
            list.addAll(dbContentRetrieve.getContent());//Add dbList to list
...
//      Retrieve List records from XML file
        ContentRetrieve xmlContentRetrieve = new XMLContentRetrieve();
        try {
            list.addAll(xmlContentRetrieve.getContent());
...
//      Retrieve List records from JSON (stored as a static String)
        ContentRetrieve jsonContentRetrieve = new JsonContentRetrieve();
        list.addAll(jsonContentRetrieve.getContent());
...
//        taxoUtils.fuelConsumptionFiltering(filteredList, lowRangeValueFuelConsumption, highRangeValueFuelConsumption);
        filteredList = taxoUtils.fuelConsumptionFiltering(filteredList, lowRangeValueFuelConsumption, highRangeValueFuelConsumption);
        System.out.println();
